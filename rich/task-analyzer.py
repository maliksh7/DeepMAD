...
with Live(progress_group):
    ...
    for idx, (name, step_times) in enumerate(apps):
        ...
        top_descr = "[bold #AAAAAA](%d out of %d tasks configured)" % (
            idx, len(apps))
        ...
        current_app_progress.update(
            current_task_id, description="[bold green]Task %s configured!" % name
        )
    ...
    overall_progress.update(
        overall_task_id, description="[bold green]%s tasks configured, done!" % len(
            apps)
    )
